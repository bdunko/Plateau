...
namespace Plateau.Entities
{
    public class TEntityMailbox : TileEntity, IInteract, ITickDaily
    {
        ...
        public TEntityMailbox(Texture2D texture, Texture2D boxGreyscale, Texture2D exclaimation, Vector2 tilePosition, int tileWidth, int tileHeight) : base(tilePosition, tileWidth, tileHeight, DrawLayer.NORMAL)
        {
            ...
            {
                RECOLOR_DIALOGUE = new DialogueNode("Should I paint my mailbox?", DialogueNode.PORTRAIT_BAD);
            ...
            if (LETTERS == null)
            {
                ...
                LETTERS.Add(new Letter(GameState.FLAG_LETTER_GIFT_CAMUS, new DialogueNode("\"<NAME>, I ordered too many metal bars in my last shipment. I can't use this one, so please take it.\"\n-Camus", DialogueNode.PORTRAIT_BAD, (player, area, world) => {
                    area.AddEntity(new EntityItem(CAMUS_GIFTS[Util.RandInt(0, CAMUS_GIFTS.Length - 1)], new Vector2(position.X, position.Y - 10)));
                })));
                LETTERS.Add(new Letter(GameState.FLAG_LETTER_GIFT_THEO, new DialogueNode("\"Dearest <NAME>, I have created this masterpiece of beauty most sublime. Feast your eyes upon it!\"\n-Theodore", DialogueNode.PORTRAIT_BAD, (player, area, world) => {
                    area.AddEntity(new EntityItem(THEO_GIFTS[Util.RandInt(0, THEO_GIFTS.Length - 1)], new Vector2(position.X, position.Y - 10)));
                })));
                LETTERS.Add(new Letter(GameState.FLAG_LETTER_GIFT_ELLE, new DialogueNode("\"Valued customer <NAME>, Nimbus General would like to offer you a free sample.\nNimbus General: For all your Nimbus needs!\n-Elle", DialogueNode.PORTRAIT_BAD, (player, area, world) => {
                    Item gift;
                ...
                LETTERS.Add(new Letter(GameState.FLAG_LETTER_GIFT_CHARLOTTE, new DialogueNode("\"Dear <NAME>, I was thinking about you when I came across this item. I believe that it was a destined to be yours.\"\n-Charlotte", DialogueNode.PORTRAIT_BAD, (player, area, world) => {
                    area.AddEntity(new EntityItem(CHARLOTTE_GIFTS[Util.RandInt(0, CHARLOTTE_GIFTS.Length - 1)], new Vector2(position.X, position.Y - 10)));
                })));
                LETTERS.Add(new Letter(GameState.FLAG_LETTER_GIFT_SKYE, new DialogueNode("\"<NAME>, I made this for you. I hope you like it.\"\n-Skye", DialogueNode.PORTRAIT_BAD, (player, area, world) => {
                    area.AddEntity(new EntityItem(SKYE_GIFTS[Util.RandInt(0, SKYE_GIFTS.Length - 1)], new Vector2(position.X, position.Y - 10)));
                })));
            }
        }
        ...
        public void InteractRight(EntityPlayer player, Area area, World world)
        {
            if (haveMail)
            {
                ...
                DialogueNode header = new DialogueNode("The mailbox has " + numLetters + (numLetters == 1 ? "letter" : " letters")  + " today!", DialogueNode.PORTRAIT_BAD);
                header.SetNext(root);
